...
from helper.loops import train_distill as validate
...
def parse_option():
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}_r:{}_a:{}_b:{}_{}'.format(opt.model_s,
...
def main():
    ...
    for epoch in range(1, opt.epochs + 2):
        ...
        for idx, data in enumerate(train_loader):
            ...
            target_all = target.unsqueeze(1).expand(
                batch, mixup_num).reshape(-1)

            loss_cls_nor = criterion_cls(logit_s_nor, target)
            loss_cls_all = criterion_cls(logit_s, target_all)
            if opt.alpha > 0:
            ...
            loss_div_all = criterion_div(logit_s, logit_t)

            if opt.gamma > 10:
                loss = loss_cls_all + opt.alpha * loss_div_all \
                    + opt.beta * loss_kd
            else:
                loss = opt.gamma * loss_cls_nor + opt.alpha * loss_div_nor \
                    + opt.beta * loss_kd + opt.alpha_aug * loss_div_aug
            # get metric
            acc1, acc5 = accuracy(logit_s_nor, target, topk=(1, 5))
            losses.update(loss.item(), input.size(0))
            losses_cls.update(loss_cls_nor.item(), input.size(0))
